...
const Header = () => {
  ...
  // We use navigate() directly instead of manipulating window.location
  ...
  const handleSearch = (e) => {
    ...
    // Fix: Use navigate with query params correctly
    // This pushes the new URL to React Router history
        ...
        <div className="left-section">
          <h1 className="logo-link">
            <Link to={token ? "/issues" : "/login"} className="logo-inner">
               {/* Simplified Logo for Pastel Theme */}
          ...
          {token ? (
            <>
              ...
              <div className="avatar-wrap" title={user?.username}>
                {user?.avatarUrl ? <img src={user.avatarUrl} alt="avatar" style={{width:'100%'}}/> : getInitials(user?.username)}
              </div>
